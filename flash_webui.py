...
прямо из контейнера (без физического доступа к плате), с построчным логом
avrdude, который стримится в браузер по мере поступления.

Подключается к уже существующему Flask-приложению вызовом:

    register_flash_routes(app, serial_port, flashing_event)

serial_port     - путь к порту платы (тот же SERIAL_PORT, что использует
                   главный цикл shkaf_stats_bridge.py).
flashing_event  - threading.Event, общий с главным циклом: пока установлен,
                   ...
                   (см. shkaf_stats_bridge.py) - чтобы два процесса не
                   дрались за один и тот же USB-порт одновременно.
"""
...
HEX_UPLOAD_PATH = "/tmp/shkaf-hud-flash.hex"
...
<html><head><meta charset="utf-8"><title>shkaf-hud - прошивка</title>
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="warn">
      Во время прошивки основной обмен с платой (лента/OLED) приостанавливается
      и возобновится автоматически после перезагрузки платы. Не отключай Pro Micro
      и не перезапускай контейнер, пока идёт заливка.
    </div>
  </div>
...
def register_flash_routes(app, serial_port, flashing_event):
    ...
    def api_flash():
        ...
        def generate():
            flashing_event.set()
            try:
                for line in flash.flash(HEX_UPLOAD_PATH, serial_port, cancel_event=cancel_event):
                    yield line + "\n"
