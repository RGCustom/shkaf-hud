...
class ProMicroTester:
    def __init__(self, root):
        ...
        for i in range(4):
            ...
            self.pct_label = ttk.Label(bar_frame, text="0%")
            ...
            # Цвета
            ttk.Label(bar_frame, text="C1 (0%):").pack(anchor=tk.W)
            c1_var = tk.StringVar(value=default_colors[0])
            ttk.Entry(bar_frame, textvariable=c1_var, width=8).pack(anchor=tk.W)
            
            ttk.Label(bar_frame, text="C2 (50%):").pack(anchor=tk.W)
            c2_var = tk.StringVar(value=default_colors[1])
            ttk.Entry(bar_frame, textvariable=c2_var, width=8).pack(anchor=tk.W)
            
            ttk.Label(bar_frame, text="C3 (100%):").pack(anchor=tk.W)
            c3_var = tk.StringVar(value=default_colors[2])
            ttk.Entry(bar_frame, textvariable=c3_var, width=8).pack(anchor=tk.W)
            ...
            self.bars.append({
                ...
                'c3': c3_var,
                'solid': solid_var,
                'pct_label': self.pct_label  # сохраняем ссылку на label
            })
    ...
    def on_bar_slider_change(self, bar_index):
        ...
        # Отправляем ТОЛЬКО этот бар (остальные не трогаем - они хранятся в памяти Arduino)
    ...
    def set_bar_and_send(self, pct_var, bar_index, value):
        ...
        cmd = f"BAR{bar_index+1}:{value},{c1},{c2},{c3},{solid}"
        self.send_cmd(cmd)
