...
КЛЮЧЕВОЕ ПРАВИЛО:
Если хоть одна переменная в шаблоне не резолвится (возвращает None - например,
net2 не выбран, или это индекс за пределами активных стримов) - render() 
возвращает all_resolved=False. Это сигнал для screens.py НЕ показывать такой экран.
"""
import re
import variables
...
def render(template: str, context: dict, index=None):
    ...
    for tok in tokens:
        if tok[0] == "text":
            ...
            if value is None:
                all_resolved = False  # КРИТИЧНО: экран будет скрыт, если хоть одна переменная None
                out.append("")
            else:
...
def template_group(template: str):
    ...
    либо None, если шаблон использует только скалярные переменные. 
    Если в одном шаблоне намешаны переменные из РАЗНЫХ повторяющихся групп - 
    возвращаем множество для диагностики (это ошибка конфигурации).
    """
    groups = set()
