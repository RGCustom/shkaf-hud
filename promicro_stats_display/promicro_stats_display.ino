/*
  ...
  Основная прошивка теперь promicro_stats_display_rus/promicro_stats_display_rus.ino
  (кириллица + русский текст). Этот файл ещё ждёт старый формат BAR
  ("pct,c1,c2,c3,solid"), а сервер (shkaf_stats_bridge.py) с версии с
  server-side градиентом всегда шлёт НОВЫЙ формат - 48 hex-символов готовых
  пикселей (см. protocol.py / ledbar.py). Если прошить эту версию - бары
  на плате будут парситься неверно (мусор вместо цветов). НЕ ПРОШИВАТЬ,
  оставлено в репозитории только для истории.
  ...
  Протокол (СТАРЫЙ, см. пометку выше - в актуальном сервере больше не используется):
