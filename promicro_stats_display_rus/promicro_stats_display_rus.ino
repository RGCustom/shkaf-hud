...
BAR1:<72 hex-символа>   - 12 пикселей x RRGGBB, ПОДРЯД, без разделителей.
BAR2:<72 hex-символа>     Цвет каждого светодиода уже посчитан на сервере
BAR3:<72 hex-символа>     (градиент/solid-режим/яркость - вся эта логика
BAR4:<72 hex-символа>     ушла с платы в Python). Плата просто раскладывает
                          готовые 12 цветов по своим физическим диодам бара
                          через LED_MAP - никакого blend()/интерполяции тут
                          больше нет.
BRI:<0-100>
CON:<0-255>              - контраст OLED (u8g2.setContrast), по умолчанию 255
L1:<текст UTF-8>
L2:<текст UTF-8>
L3:<текст UTF-8>

Пример (BAR1 - 12 пикселей, первые 6 зелёные, дальше жёлтый/оранжевый, потом
6 потушенных):
BAR1:00FF0000FF0000FF0040FF0080FF00FFFF00FF8000000000000000000000000000000000|L1:Шкаф HUD
...
// ---------------- КОНФИГ ----------------
#define LED_PIN        6
#define LEDS_PER_BAR   12
#define NUM_BARS       4
#define NUM_LEDS       (LEDS_PER_BAR * NUM_BARS)   // 48
...
// Третья матрица (LEDS_PER_BAR 8 -> 12) удлинила каждый BAR-параметр с 48 до
// 72 hex-символов. Точный худший случай - полный ресинк (все 9 полей:
// 4xBAR по 77 символов "BARn:"+72hex, BRI, CON, 3xL по OLED_LINE_BUF-1 символов,
// разделители "|") - около 530 байт, буфер увеличен до 550 с небольшим
// запасом.
#define SERIAL_BUF_SIZE 550
...
// Добавлена третья матрица (LEDS_PER_BAR 8 -> 12, NUM_LEDS 32 -> 48) -
// старая калиброванная раскладка была только под 32 диода и для новой
// физической цепочки не подходит. Пришли "CAL" в Serial Monitor, запиши
// новый порядок raw-индексов по каждому бару (см. runCalibration()) и
// замени этот массив-заглушку (сейчас просто identity-маппинг).
// Храним в PROGMEM, чтобы не занимать RAM.
const uint8_t LED_MAP[NUM_LEDS] PROGMEM = {
   0,  1,  2,  3,  16,  17,  18,  19,  32,  33, 34, 35,   // бар 1 (низ -> верх) - ЗАГЛУШКА
  7, 6, 5, 4, 23, 22, 21, 20, 39, 38, 37, 36,   // бар 2 - ЗАГЛУШКА
  8, 9, 10, 11, 24, 25, 26, 27, 40, 41, 42, 43,   // бар 3 - ЗАГЛУШКА
  15, 14, 13, 12, 31, 30, 29, 28, 47, 46, 45, 44    // бар 4 - ЗАГЛУШКА
};
...
// про solid-режим - только про 48 конкретных цвета. Глобальные массивы
...
// val - 72 hex-символа подряд (12 пикселей x RRGGBB), без разделителей.
// parseHex6 сам продвигает указатель ровно на 6 hex-цифр за вызов, так что
// достаточно просто дёрнуть его 12 раз. Если данные пришли обрезанными -
