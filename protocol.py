...
Собирает serial-строку для Arduino из МИНИМАЛЬНОГО набора переменных:
    ...
    BAR1, BAR2, BAR3, BAR4  - готовый цвет КАЖДОГО светодиода бара, посчитанный
                               на сервере (см. ledbar.py): 8 пикселей x RRGGBB,
                               склеенные подряд без разделителей = 48 hex-символов.
...
9 переменных, при этом каждая может быть длинной - вместо прежних ~20 мелких
...
def pack_bar_pixels(pixels):
    """
    pixels: список из LEDS_PER_BAR (обычно 8) строк 'RRGGBB' (без '#'),
    ...
    -> '00FF0000FF0040FF0080FF00FFFF00FF8000000000000000' (48 символов при 8 LED)
    """
    for px in pixels:
        if len(px) != 6:
